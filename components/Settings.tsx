...
const Settings: React.FC<SettingsProps> = ({ isDarkMode, toggleTheme, confirmAction, onClearData }) => {
  const sections = [
    {
      title: 'Profile',
      items: [
      ...
      <div className="space-y-8 pb-12">
            ...
            <div className="bg-[var(--bg-secondary)] organic-shape organic-border subtle-depth overflow-hidden">
              {section.items.map((item, i) => (
                <div 
                  key={item.id} 
                  onClick={item.action}
                  className={`flex items-center justify-between p-6 hover:bg-[var(--bg-primary)]/50 transition-organic cursor-pointer ${
        ...
        <div className="pt-8">
          ...
          <div className="text-center space-y-2 mt-8">
             <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase tracking-widest opacity-50">Local Deployment v1.0.4</p>
             <p className="text-[10px] text-[var(--text-secondary)] leading-relaxed px-4">
               Your privacy is paramount. FitFlow Pro stores 100% of your training metrics on your device. We do not transmit health data to our servers.
             </p>
          </div>
        </div>
      </div>
