...
const AICoach: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    const coachMsg: Message = { id: (Date.now() + 1).toString(), role: 'coach', text: response || '' };
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map((m) => (
            <div key={m.id} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-10 h-10 shrink-0 organic-shape flex items-center justify-center border subtle-depth ${
                m.role === 'coach' ? 'bg-[#8b947a]/10 border-[#8b947a]/30 text-[#8b947a]' : 'bg-[#a3a395]/10 border-[#a3a395]/30 text-[#a3a395]'
              }`}>
