...
import { WeeklyPlan, MuscleGroup } from '../types';
...
const WeeklySchedule: React.FC<WeeklyScheduleProps> = ({ weeklyPlan, setWeeklyPlan, confirmAction }) => {
  const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
  const [editingDay, setEditingDay] = useState<string | null>(null);

  const removeExercise = (day: string, exIndex: number) => {
    confirmAction(
      "Remove Exercise?",
  ...
  const addExercise = (day: string) => {
    const newEx = { name: 'New Exercise', sets: 3, reps: 10, muscleGroup: 'Core' as MuscleGroup };
    setWeeklyPlan(prev => ({
      ...prev,
      [day]: {
        ...prev[day],
